...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!reply) reply = buildFallbackReply(message, stats, contacts);
...
function buildFallbackReply(message, stats, contacts) {
  ...
  if (lower.includes('call') || lower.includes('who')) {
    ...
    return names
      ? `Start with ${names}. Call in that order, log the outcome immediately, then set a follow-up date before moving on.`
      : 'Add or import contacts first. Once contacts are in, your Power List will tell you exactly who to call.';
  }
  if (lower.includes('all-the-timer') || lower.includes('timer')) {
    const gap = Math.max(0, 21 - stats.pts);
    return gap === 0
      ? 'You are already at All-the-Timer. Protect the week by logging every extra contact, FNA, and recruit touch.'
      : `You need ${gap} more points for All-the-Timer. Fastest path: finish the next 10-contact block, then chase one FNA or new partner.`;
  }
  return `Right now you have ${stats.pts} points and ${stats.totalContacts} contacts. Your clearest next move is one focused 10-contact block, then follow up with the hottest person on the Power List.`;
}
